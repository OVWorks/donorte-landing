...
const fragmentShader = `
  ...
  void main() {
    ...
    vec3 color = mix(uColorA, uColorB, fresnel + sin(uTime * 0.5) * 0.1);

    // Add glow effect
    float glow = fresnel * 0.5;
    color += vec3(0.16, 0.44, 1.0) * glow;
...
export default function BlobMesh() {
  ...
  const uniforms = useMemo(
    () => ({
      ...
      uColorA: { value: new THREE.Color('#2970ff') },
      uColorB: { value: new THREE.Color('#1a4dcc') },
    }),
    []
  );
